const contactLinks = [
  {
    id: "github",
    label: "GitHub",
    href: "https://github.com/gustj369",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z" />
      </svg>
    ),
  },
  {
    id: "email",
    label: "메일로 연락하기",
    href: "mailto:gustj258@gmail.com",
    icon: (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
          d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
      </svg>
    ),
  },
  {
    id: "instagram",
    label: "Instagram",
    href: "https://www.instagram.com/94hyeonseo",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
      </svg>
    ),
  },
  {
    id: "threads",
    label: "Threads",
    href: "https://www.threads.com/@94hyeonseo",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12.186 24h-.007c-3.581-.024-6.334-1.205-8.184-3.509C2.35 18.44 1.5 15.586 1.472 12.01v-.017c.03-3.579.879-6.43 2.525-8.482C5.852 1.205 8.6.024 12.18 0h.014c2.746.02 5.043.725 6.826 2.098 1.677 1.29 2.858 3.13 3.509 5.467l-2.04.569c-1.104-3.96-3.898-5.984-8.304-6.015-2.91.022-5.11.936-6.54 2.717C4.307 6.504 3.616 8.914 3.589 12c.027 3.086.718 5.496 2.057 7.164 1.43 1.783 3.631 2.698 6.54 2.717 2.623-.02 4.358-.631 5.8-2.045 1.647-1.613 1.618-3.593 1.09-4.798-.31-.71-.873-1.3-1.634-1.75-.192 1.352-.622 2.446-1.284 3.272-.886 1.102-2.14 1.704-3.73 1.79-1.202.065-2.361-.218-3.259-.801-1.063-.689-1.685-1.74-1.752-2.964-.065-1.19.408-2.285 1.33-3.082.88-.76 2.119-1.207 3.583-1.291a13.853 13.853 0 012.076.175V12c0-.91-.244-1.612-.753-2.129-.51-.517-1.24-.78-2.211-.801-1.275.025-2.22.625-2.67 1.478l-1.776-1.075c.713-1.348 2.105-2.157 3.976-2.226 1.64.001 2.98.489 3.908 1.41.903.896 1.372 2.16 1.372 3.68v.1c.493.23.96.497 1.393.806 1.34.96 2.152 2.336 2.43 3.907.375 2.104-.175 4.29-1.647 5.96-1.701 1.93-4.137 2.836-7.36 2.86h-.007z" />
      </svg>
    ),
  },
];

const openToItems = ["Portfolio", "Automation", "Content", "AI Workflow"];

export default function Contact() {
  return (
    <section id="contact" className="py-24 md:py-32 px-6">
      <div className="max-w-6xl mx-auto">
        <p className="text-xs font-mono text-gold-500 tracking-[0.3em] uppercase mb-4">
          Contact
        </p>
        <div className="section-divider mb-16" />

        {/* 2-column layout */}
        <div className="grid md:grid-cols-[1fr_auto] gap-12 md:gap-16 items-start">

          {/* Left: Title + description + buttons */}
          <div className="max-w-xl">
            <h2 className="text-3xl md:text-4xl font-semibold leading-snug mb-6">
              같이 만들고 싶은 게 있다면
              <br />
              <span className="text-gradient-gold">편하게 이야기해주세요.</span>
            </h2>
            <p className="text-gray-400 text-base leading-relaxed mb-10">
              작은 아이디어를 웹사이트, 콘텐츠, AI 활용 도구로 구현하는 일을 좋아합니다.
              <br />
              함께 만들고 싶은 프로젝트나 협업 아이디어가 있다면 편하게 연락해주세요.
            </p>

            {/* Buttons */}
            <div className="grid grid-cols-2 sm:flex sm:flex-row sm:flex-wrap gap-3">
              {contactLinks.map((link) => {
                const isEmail = link.id === "email";
                return (
                  <a
                    key={link.id}
                    href={link.href}
                    target={isEmail ? undefined : "_blank"}
                    rel={isEmail ? undefined : "noopener noreferrer"}
                    aria-label={isEmail ? `메일로 연락하기: gustj258@gmail.com` : link.label}
                    className={[
                      "inline-flex items-center justify-center gap-2.5 px-5 py-3 rounded-xl border text-sm",
                      "transition-all duration-200 group",
                      "hover:-translate-y-0.5",
                      isEmail
                        ? "border-gold-500/50 text-gold-400 hover:border-gold-400 hover:bg-gold-500/8 hover:shadow-[0_0_16px_rgba(212,168,67,0.12)]"
                        : "border-dark-400 text-gray-400 hover:border-gold-500/40 hover:text-gold-400",
                    ].join(" ")}
                  >
                    <span className="flex-shrink-0 transition-transform duration-200 group-hover:scale-110">
                      {link.icon}
                    </span>
                    <span className="truncate">{link.label}</span>
                  </a>
                );
              })}
            </div>

            {/* Email address hint */}
            <p className="mt-4 text-xs font-mono text-gray-700">
              gustj258@gmail.com
            </p>
          </div>

          {/* Right: OPEN TO card */}
          <div className="md:w-52 md:pt-2">
            <div className="rounded-xl border border-dark-500/50 bg-dark-800/40 p-5">
              <p className="text-xs font-mono text-gold-500/80 tracking-widest mb-4">
                OPEN TO
              </p>
              <div className="flex flex-wrap gap-2">
                {openToItems.map((item) => (
                  <span
                    key={item}
                    className="px-2.5 py-1 text-xs rounded-lg border border-dark-400/60 text-gray-500 bg-dark-700/30"
                  >
                    {item}
                  </span>
                ))}
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* Footer */}
      <div className="max-w-6xl mx-auto mt-24 pt-8 border-t border-dark-600/50">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-3 text-xs text-gray-600">
          <span className="font-mono text-gold-600/60 tracking-widest">
            DEV · CREATOR
          </span>
          <span>
            Turning small ideas into real interfaces · {new Date().getFullYear()}
          </span>
        </div>
      </div>
    </section>
  );
}
